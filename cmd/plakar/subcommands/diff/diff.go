...
func diff_files(snap1 *snapshot.Snapshot, fileEntry1 *vfs.FileEntry, snap2 *snapshot.Snapshot, fileEntry2 *vfs.FileEntry) (string, error) {
	...
	if fileEntry1.Object.Checksum == fileEntry2.Object.Checksum {
		fmt.Printf("%s:%s and %s:%s are identical\n",
			fmt.Sprintf("%x", snap1.Header.GetIndexShortID()), filepath.Join(fileEntry1.ParentPath, fileEntry1.Info.Name()),
			fmt.Sprintf("%x", snap2.Header.GetIndexShortID()), filepath.Join(fileEntry2.ParentPath, fileEntry2.Info.Name()))
		return "", nil
	}

	filename1 := filepath.Join(fileEntry1.ParentPath, fileEntry1.Info.Name())
	filename2 := filepath.Join(fileEntry2.ParentPath, fileEntry2.Info.Name())
