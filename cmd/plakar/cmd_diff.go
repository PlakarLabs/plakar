...
func cmd_diff(ctx Plakar, args []string) int {
	...
	if len(flags.Args()) == 2 {
		...
		if !rel1 {
			...
			for dir1 := range snapshot1.Directories {
				fi1, _ := snapshot1.GetInode(dir1)
			...
			for dir2 := range snapshot2.Directories {
				fi2, _ := snapshot2.GetInode(dir2)
			...
			for file1 := range snapshot1.Files {
				fi1, _ := snapshot1.GetInode(file1)
			...
			for file2 := range snapshot2.Files {
				fi2, _ := snapshot1.GetInode(file2)
