...
func New(store *storage.Store, secret []byte) (*Repository, error) {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "New(store=%p): %s", store, time.Since(t0))
	}()
...
func (r *Repository) RebuildState() error {
	...
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "rebuildState(): %s", time.Since(t0))
	}()
...
func (r *Repository) Close() error {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "Close(): %s", time.Since(t0))
	}()
...
func (r *Repository) Decode(buffer []byte) ([]byte, error) {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "Decode(%d bytes): %s", len(buffer), time.Since(t0))
	}()
...
func (r *Repository) Encode(buffer []byte) ([]byte, error) {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "Encode(%d): %s", len(buffer), time.Since(t0))
	}()
...
func (r *Repository) GetSnapshots() ([]objects.Checksum, error) {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "GetSnapshots(): %s", time.Since(t0))
	}()
...
func (r *Repository) DeleteSnapshot(snapshotID objects.Checksum) error {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "DeleteSnapshot(%x): %s", snapshotID, time.Since(t0))
	}()
...
func (r *Repository) GetStates() ([]objects.Checksum, error) {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "GetStates(): %s", time.Since(t0))
	}()
...
func (r *Repository) GetState(checksum objects.Checksum) ([]byte, int64, error) {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "GetState(%x): %s", checksum, time.Since(t0))
	}()
...
func (r *Repository) PutState(checksum objects.Checksum, rd io.Reader, size int64) (int, error) {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "PutState(%x, ...): %s", checksum, time.Since(t0))
	}()
...
func (r *Repository) DeleteState(checksum objects.Checksum) error {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "DeleteState(%x, ...): %s", checksum, time.Since(t0))
	}()
...
func (r *Repository) GetPackfiles() ([]objects.Checksum, error) {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "GetPackfiles(): %s", time.Since(t0))
	}()
...
func (r *Repository) GetPackfile(checksum objects.Checksum) (io.Reader, uint64, error) {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "GetPackfile(%x, ...): %s", checksum, time.Since(t0))
	}()
...
func (r *Repository) GetPackfileBlob(checksum objects.Checksum, offset uint32, length uint32) (io.Reader, int64, error) {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "GetPackfileBlob(%x, %d, %d): %s", checksum, offset, length, time.Since(t0))
	}()
...
func (r *Repository) PutPackfile(checksum objects.Checksum, rd io.Reader, size uint64) error {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "PutPackfile(%x, ...): %s", checksum, time.Since(t0))
	}()
...
func (r *Repository) DeletePackfile(checksum objects.Checksum) error {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "DeletePackfile(%x): %s", checksum, time.Since(t0))
	}()
...
func (r *Repository) GetBlob(Type packfile.Type, checksum objects.Checksum) (io.Reader, uint64, error) {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "GetBlob(%x): %s", checksum, time.Since(t0))
	}()
...
func (r *Repository) BlobExists(Type packfile.Type, checksum objects.Checksum) bool {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "BlobExists(%x): %s", checksum, time.Since(t0))
	}()
...
func (r *Repository) ListSnapshots() <-chan objects.Checksum {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "ListSnapshots(): %s", time.Since(t0))
	}()
...
func (r *Repository) SetPackfileForBlob(Type packfile.Type, packfileChecksum objects.Checksum, chunkChecksum objects.Checksum, offset uint32, length uint32) {
	t0 := time.Now()
	defer func() {
		logging.Trace("repository", "SetPackfileForBlob(%x, %x, %d, %d): %s", packfileChecksum, chunkChecksum, offset, length, time.Since(t0))
	}()
