...
func archiveTarball(snap *snapshot.Snapshot, out io.Writer, fs *vfs.Filesystem, path string, rebase bool) error {
	...
	for file := range fs.Pathnames() {
		...
		if err != nil {
			logging.Error("could not stat file %s: %s", file, err)
			continue
		}
		...
		default:
			logging.Error("could not stat file %T: %s %s", file, file, err)
		}
		...
		if err != nil {
			logging.Error("could not write header for file %s: %s", file, err)
			rd.Close()
			continue
		}
		...
		if err != nil {
			logging.Error("could not write file %s: %s", file, err)
			rd.Close()
			return err
		}
		rd.Close()
