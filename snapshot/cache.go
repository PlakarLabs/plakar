...
func (snapshot *Snapshot) GetCachedObject(pathname string) (*CachedObject, error) {
	...
	data, err := cache.GetPath(snapshot.repository.Configuration().Uuid.String(), hashedPath)
	if err != nil {
...
func (snapshot *Snapshot) PutCachedObject(pathname string, object Object, fi filesystem.Fileinfo) error {
	...
	cache.PutPath(snapshot.repository.Configuration().Uuid.String(), hashedPath, jobject)
	return nil
}
