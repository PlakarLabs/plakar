...
func info_vfs(repo *repository.Repository, snapshotPath string) error {
	...
	if dirEntry, isDir := fsinfo.(*vfs.DirEntry); isDir {
		...
		fmt.Printf("Type: %d\n", dirEntry.Type)
		fmt.Printf("Size: %s (%d bytes)\n", humanize.Bytes(uint64(dirEntry.FileInfo().Size())), dirEntry.FileInfo().Size())
		fmt.Printf("Permissions: %s\n", dirEntry.FileInfo().Mode())
		fmt.Printf("ModTime: %s\n", dirEntry.FileInfo().ModTime())
		fmt.Printf("DeviceID: %d\n", dirEntry.FileInfo().Dev())
		fmt.Printf("InodeID: %d\n", dirEntry.FileInfo().Ino())
		fmt.Printf("UserID: %d\n", dirEntry.FileInfo().Uid())
		fmt.Printf("GroupID: %d\n", dirEntry.FileInfo().Gid())
		...
		fmt.Printf("AggregateSize: %s (%d bytes)\n", humanize.Bytes(uint64(dirEntry.AggregatedStats.Size)), dirEntry.AggregatedStats.Size)
		...
		fmt.Printf("Type: %d\n", fileEntry.Type)
		fmt.Printf("Size: %s (%d bytes)\n", humanize.Bytes(uint64(fileEntry.FileInfo().Size())), fileEntry.FileInfo().Size())
		fmt.Printf("Permissions: %s\n", fileEntry.FileInfo().Mode())
		fmt.Printf("ModTime: %s\n", fileEntry.FileInfo().ModTime())
		fmt.Printf("DeviceID: %d\n", fileEntry.FileInfo().Dev())
		fmt.Printf("InodeID: %d\n", fileEntry.FileInfo().Ino())
		fmt.Printf("UserID: %d\n", fileEntry.FileInfo().Uid())
		fmt.Printf("GroupID: %d\n", fileEntry.FileInfo().Gid())
		fmt.Printf("NumLinks: %d\n", fileEntry.FileInfo().Nlink())
