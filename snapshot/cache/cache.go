...
func (c *Cache) LookupFileSummary(origin string, filename string) (*vfs.FileSummary, [32]byte, uint64, error) {
	...
	key := []byte(fmt.Sprintf("__file_summary__:%s:%x", encodedOrigin, hashedFilename))
	...
	err = msgpack.Unmarshal(data, &fileSummary)
	if err != nil {
		return nil, [32]byte{}, 0, err
	}

	return &fileSummary, sha256.Sum256(data), uint64(len(data)), nil
}

// RecordFilename stores the FileInfo in LevelDB using the pathname as the key.
func (c *Cache) RecordFileSummary(origin string, filename string, fileSummary *vfs.FileSummary) error {
	...
	// Serialize the FileInfo to JSON
	data, err := msgpack.Marshal(fileSummary)
	if err != nil {
		return err
	}
